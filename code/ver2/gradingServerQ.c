...
// initialize a global thread ID
int thread_no = 0;
pthread_mutex_t lock; 
// starter function for each tread
void *thread_start(void *mysockfd){
  pthread_mutex_lock(&lock); 
    int local_thread = thread_no++;
  pthread_mutex_unlock(&lock); 
  int newsockfd = *(int *)mysockfd;
  // create filenames and commands for each thread
  printf("starting thread %d\n", local_thread);
  char filename[50];          sprintf(filename, "file_%d.cpp", local_thread);
  char compile_command[100];  sprintf(compile_command, "g++ -w %s 2> compile_%d.txt", filename, local_thread);
  char runtime_command[100];  sprintf(runtime_command, "./a.out > out_%d.txt 2> runtime_%d.txt", local_thread, local_thread);
  char diff_command[100];     sprintf(diff_command, "diff out_%d.txt output.txt > diff_%d.txt", local_thread, local_thread);
  ...
  int n;
  if (system(compile_command) !=0 )       n = send(newsockfd, "COMPILER ERROR\n", 15, 0);
 	else if ( system(runtime_command) !=0 ) n = send(newsockfd, "RUNTIME ERROR\n", 14, 0);
 	else if( system(diff_command) != 0 )    n = send(newsockfd, "PROGRAM RAN WRONG OUTPUT\n", 25, 0);
...
int main(int argc, char *argv[]) {
  ...
  while (1){
    ...
    pthread_t mythread;
    if(pthread_create(&mythread, NULL, thread_start, &newsockfd))
      error("ERROR : thread creation");   
  }
  return 0;
}
